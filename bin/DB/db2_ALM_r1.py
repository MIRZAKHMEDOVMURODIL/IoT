#########################################################
# Update:2023/1/19
# program Name:db2_ALM.py
# Process:IOT_DATA.ALARMテーブルへアラームデータを保存（ファイルフォーマットは全設備共通(三菱タッチパネルの標準出力アラームcsv))
...
if rst == 0:
    ...
    #INSERT対象の列名
    column_insert = ['EQP_ID', 'UNIT_NUM', 'UNIT_NAME', 'ALM_CODE', 'ALM_MESSAGE', 'OCCURRED_DT', 'RELEASED_DT', 'PASSED_TIME']
    ...
    for filepath in files:
        ...
        try:
            ...
            # 時間の差分を計算して秒に変換
            df_csv['diff_time'] = pd.to_datetime(df_csv['RESTORED'],errors='coerce') - pd.to_datetime(df_csv['OCCURRED'],errors='coerce')
            df_csv['passed_time'] = df_csv['diff_time'].map(lambda x: x.total_seconds())

            # 新しいデータフレームに必要な情報を抜粋してゆく
            df_buff =pd.DataFrame({})

            df_buff['ALM_CODE'] = (df_csv["COMMENT_NO"])
            df_buff['ALM_MESSAGE'] = (df_csv["COMMENT"])
            df_buff['OCCURRED_DT'] = (pd.to_datetime(df_csv['OCCURRED'], format='%Y/%m/%d %H:%M:%S')).dt.strftime('%Y-%m-%d %H:%M:%S') #DB2登録用のDate文字列を設定
            df_buff['RELEASED_DT'] = (pd.to_datetime(df_csv['RESTORED'], format='%Y/%m/%d %H:%M:%S')).dt.strftime('%Y-%m-%d %H:%M:%S') #DB2登録用のDate文字列を設定
            df_buff['PASSED_TIME'] = (df_csv["passed_time"])
            ...
            # 最終データの更新日時を取得
            if measured_dt_latest < df_buff['OCCURRED_DT'].max():
                measured_dt_latest = df_buff['OCCURRED_DT'].max()
            ...
            sql_insert_under = " ON ("\
                                ...
                                " AND TGT.ALM_CODE = INSERTDATA.ALM_CODE"\
...
if rst == 0:
    ...
    try:
        ...
        column_index_tmp = ['EQP_ID', 'UNIT_NUM', 'ALM_CODE', 'OCCURRED_DT']
