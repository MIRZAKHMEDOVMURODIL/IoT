#########################################################
# Update:2023/1/19
# program Name:db2_STS.py
# Process:IOT_DATA.STATUSテーブルへステータスデータを保存（ファイルフォーマットは全設備共通(三菱タッチパネルの標準出力ロギングcsv))
...
if rst == 0:
    try:
        ...
        logger.info("************" + eqp_id + " " + unit_num + " " + unit_name + " " + Dtype + ("" if log_num == "" else ("_" + str(log_num))) + "の書き込み START*******************")
...
if rst == 0:
    ...
    for filepath in files:
        ...
        try:
            ...
            #不要な行を削除
            # df_buff1 =df_buff1.drop(['UPPER_NO','MIDDLE_NO','CHECKED','LEVEL','GROUP',"STATUS","date_diff"],axis=1)
            ...
            # 最終データの更新日時とコメントを取得
            comment_latest = (df_buff['STATUS_CONTENTS'][-1:].values)[0]
            comment_no_latest = (df_buff['STATUS_CODE'][-1:].values)[0]
...
if rst > 1 or rst == 0:
    ...
    if rst == 2:
        ...
        msg_rst = "MERGEエラー"
    elif int(comment_no_latest) == 16: #ログ無しのコメントNo
        rst = 6 #ログ無し(PLCリンクが切れている等)
        msg_rst = comment_latest
    # elif "ログなし" in comment_latest:
    #     rst = 6 #ログ無し(PLCリンクが切れている等)
    #     msg_rst = comment_latest
