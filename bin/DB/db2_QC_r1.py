...
if rst == 0:
    try:
        ...
        logger.info("************" + eqp_id + " " + unit_num + " " + unit_name + " " + Dtype + "_" + str(log_num) + "の書き込み START*******************")
...
if rst == 0:
    ...
    column_insert = ['EQP_ID', 'UNIT_NUM', 'UNIT_NAME', 'MEASURED_DT', 'LOG_NUM']

    # 追加登録のデータがあればカラム名を追加
    if j_data["colname_recipe_code"] != None:
        colname_recipe_code = 'RECIPE_CODE'
        column_insert.append(colname_recipe_code)
        column_get_csv.append([colname_recipe_code, j_data["colname_recipe_code"]])

    if "colname_recipe_name" in j_data: #レシピ名の登録項目がある場合(2棟設備用)
        if j_data["colname_recipe_name"] != None:
            colname_recipe_name = 'RECIPE_NAME'
            column_insert.append(colname_recipe_name)
            column_get_csv.append([colname_recipe_name, j_data["colname_recipe_name"]])
    ...
    if j_data["colname_serial_num"] != None:
        colname_serial_num = 'SERIAL_NUM'
        column_insert.append(colname_serial_num)
        column_get_csv.append([colname_serial_num, j_data["colname_serial_num"]])

    if j_data["colname_parts_num"] != None:
        colname_parts_num = 'PARTS_NUM'
        column_insert.append(colname_parts_num)
        column_get_csv.append([colname_parts_num, j_data["colname_parts_num"]])

    if j_data["colname_st_time"] != None:
        colname_st_time = 'ST_TIME'
        column_insert.append(colname_st_time)
        
    if j_data["colname_end_time"] != None:
        colname_end_time = 'END_TIME'
        column_insert.append(colname_end_time)

    if j_data["colname_jig_id"] != None:
        colname_jig_id = 'JIG_ID'
        column_insert.append(colname_jig_id)
        column_get_csv.append([colname_jig_id, j_data["colname_jig_id"]])

    if j_data["colname_lot_num"] != None:
        colname_lot_no = 'LOT_NUM' #カラム名は暫定(既存例が見つからないので)
        column_insert.append(colname_lot_no) 
        column_get_csv.append([colname_lot_no, j_data["colname_lot_num"]])

    # if j_data["colname_pnjn"] != None:
    #     colname_pnjn = 'PNJN' #カラム名は暫定(既存例が見つからないので)
    #     column_insert.append(colname_pnjn) 
    #     column_get_csv.append([colname_pnjn, j_data["colname_pnjn"]])
    ...
    for filepath in files:
        ...
        try:
            ...
            # 開始、終了時刻の取得がONの場合はデータ取得（csvフォーマットによってはそもそも時刻データがないので、その場合は取得しない）
            if j_data["colname_st_time"] != None:
                # 開始時刻のデータ列を文字列結合して保存
                df_dt =pd.DataFrame({}) #空のデータフレーム作成
                column_name_tb = colname_st_time # 登録列名
                column_name_csv = j_data["colname_st_time"] #csvデータ参照列名
                column_no_csv = df_csv.columns.get_loc(column_name_csv) #参照開始列番を取得

                for No in range(0, 6): #日時データを参照（参照開始位置から6列分を参照)
                    colno = column_no_csv + No
                    if No == 0:
                        df_dt[column_name_tb] = (df_csv.iloc[:, column_no_csv].str.replace(" ",""))
                        df_dt["str_cnt"] = df_dt[column_name_tb].apply(lambda x: len(x)) # 年データが何文字で構成されているかを確認
                        df_dt.loc[df_dt.str_cnt==2, column_name_tb] = (str((datetime.date.today()).year))[:2] + df_dt[column_name_tb] #年データが末尾2桁のみなら、現在の年データから先頭2桁を追加
                        df_dt[column_name_tb] = df_dt[column_name_tb].str.zfill(4) #年データを4桁の数値にゼロ埋め(上記の2桁以外(1桁表記の場合もある)の対応のため)
                        del df_dt["str_cnt"] #年データの文字数カウント列を削除
                    if No > 0 and No < 3:
                        df_dt[column_name_tb] = (df_dt[column_name_tb].str.cat((df_csv.iloc[:, colno].str.replace(" ","")).str.zfill(2), sep = '-')) #ハイフンを間にいれて日付文字を結合(2桁数字に設定)
                    if No == 3:
                        df_dt[column_name_tb] = df_dt[column_name_tb].str.cat((df_csv.iloc[:, colno].str.replace(" ","")).str.zfill(2), sep = ' ') #日付文字列の後ろにHH文字を結合（間にスペースを挟む)
                    if No > 3 and No < 6:
                        df_dt[column_name_tb] = (df_dt[column_name_tb].str.cat((df_csv.iloc[:, colno].str.replace(" ","")).str.zfill(2), sep = ':')) #コロンを間にいれて時刻文字を結合

                # 作成したdf_dtをdf_insertに結合
                df_buff = pd.concat([df_buff, df_dt], axis=1) #列方向に結合

            if j_data["colname_end_time"] != None:
                # 終了時刻のデータ列を文字列結合して保存
                df_dt =pd.DataFrame({}) #空のデータフレーム作成

                column_name_tb = colname_end_time # 登録列名
                column_name_csv = j_data["colname_end_time"] #csvデータ参照列名
                column_no_csv = df_csv.columns.get_loc(column_name_csv) #参照開始列番を取得
            ...
            # 特定の条件で行を削除する必要がある場合はここで実行（全て列結合してからでないと、後から結合が困難）

            # 開始、終了時刻が「0000-00-00 00:00:00」の未登録の場合はその行を削除する。「yyyy-mm-dd hh:mm:ss」の場合はNULLに変換
            if j_data["colname_st_time"] != None:
                df_buff[colname_st_time] = df_buff[colname_st_time].replace('yyyy-mm-dd hh:mm:ss' ,'NULL')
                df_buff= df_buff[(df_buff[colname_st_time] != '0000-00-00 00:00:00')]

            if j_data["colname_end_time"] != None:
                df_buff[colname_end_time] = df_buff[colname_end_time].replace('yyyy-mm-dd hh:mm:ss' ,'NULL')
                df_buff = df_buff[(df_buff[colname_end_time] != '0000-00-00 00:00:00')]
